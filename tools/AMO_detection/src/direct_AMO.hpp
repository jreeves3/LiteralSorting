...
class Direct_AMO : public Extraction_engine {
    ...
    public: 
        ...
        vector<tuple<int,int>> clause_id_map_vector;
        ...
        void run (double timeout) {
            ...
            while (clause_id_map.size ()) {
                ...
                auto it = clause_id_map_vector[next_pos++];
                while (clause_id_map.find (it) == clause_id_map.end()) {
                  if (next_pos == clause_id_map_vector.size()) break;
                  it = clause_id_map_vector[next_pos++];
    ...
    void generate_maps () {
        ...
        for (int cls_idx = 0; cls_idx < cnf_extractor->clauses.size(); cls_idx++) {
            ...
            clause_id_map_vector.push_back (tuple<int,int>{l1,l2});

            // insert edge into both literals edge map
            if (find (edge_map[l1].begin(), edge_map[l1].end(), l2) == edge_map[l1].end ()) edge_map[l1].push_back (l2);
            if (find (edge_map[l2].begin(), edge_map[l2].end(), l1) == edge_map[l2].end ()) edge_map[l2].push_back (l1);

        }
    }

    void expand_clique (int lit, int lit2) {
        clique_set.clear();
        bool include;
        set<int> clique_set_local;
        clique_set_local.insert (lit);
        clique_set_local.insert (lit2);
        ...
        for (auto olit : edge_map[lit]) {
            ...
            for (auto it = clique_set_local.begin() ; it != clique_set_local.end(); it++) {
                int clit = *it;
                if (olit != clit && find(edge_map[olit].begin(), edge_map[olit].end(),clit) == edge_map[olit].end()) {
                    include = false;
                    break;
                }
            }
    ...
    void remove_edge (int l1, int l2) {
        ...
        edge_map[l1].erase (remove(edge_map[l1].begin(), edge_map[l1].end(), l2), edge_map[l1].end());
        // assert (removed);

        edge_map[l2].erase (remove(edge_map[l2].begin(), edge_map[l2].end(), l1), edge_map[l2].end());
        // assert (removed);
    ...
    void emit_AMO () {
        ...
        if (cnf_extractor->extractor_options["Quick_Write"] == "true") {
            cout << "m " << klause.cardinality_bound << " " << literals2str(klause.literals) << "0" << endl;
        }
