...
public:
  ...
  virtual void AMOConstraint(vector<int>& clause, int bound, double weight, string s_weight, int guard) {};
  ...
  void Parse(string knf_path, Input_Type input_type) override {
    ...
    while (c != EOF) {
      ...
      if(c == 'k') { // cardinality constraint
        ...
        SkipWhitespace(file, c);
      } else if(c == 'm') { // at most one constraint
        statement_type = 'm';
        c = getc_unlocked(file); // skip 'm'
        SkipWhitespace(file, c);
        ParseInteger(file, c, bound); // get bound that follows integer
        assert (bound >= 1); // bound at least 1
        SkipWhitespace(file, c);
      ...
      if (input_type != WCARD) {
        ...
        else if(statement_type == 'm') { // cardinality constraint
          ncls++;
          for(auto observer : observers_) {
            observer->AMOConstraint(clause, bound, weight, s_weight, guard);
          }
          guard = 0;
        } 
